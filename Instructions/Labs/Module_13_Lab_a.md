...
1. 「**条件**」 タイルの右上隅にある省略記号を選択し、ポップアップ メニューで、「**名前の変更**」 を選択し、**リソース グループの仮想マシンが変更された場合**、に置き換えます。 

1. 条件の左側の 「**値の選択**」 テキスト ボックスを選択し、ポップ アップ ウィンドウの 「**式**」 タブで、以下の式を入力し、「**OK**」 を選択します。

   ```
   triggerBody()?['data']['operationName']
   ```
次のの
1. **「次の値に等しい」** が条件に表示されていることを確認し、右側の 「**値の選択**」 テキスト ボックスに、監視する操作を表す値を入力します。
...
1. 「**Outlook.com**」 ウィンドウで、「**メールの送信 (V2)**」 タイルの右上隅にある省略記号を選択します。
...
1. 「**メールを送信**」 ウィンドウに、次の設定を指定し、「**保存**」 を選択します。
    ...
    | 件名 | 「**リソース更新:**」を入力して、「**動的なコンテンツ**」 で、「**件名**」 を選択します。 |
    | 本文 |以下に示すように入力します| 
        
    **リソース グループ:** と入力し、**「動的コンテンツの検索」** に **「トピック」** と入力して選択します。
    **「ボディ」** テキスト ボックスに戻り、新しい行に **イベント タイプ:** と入力し、**「動的コンテンツ」** から**イベントの種類** 選択。
    **「ボディ」** テキスト ボックスに戻り、新しい行に **イベントID:** と入力し、**「動的コンテンツ」** から **ID** を選択。
    **「ボディ」** テキスト ボックスに戻り、新しい行に **イベント時間:** を入力し、**「動的コンテンツ」** から **イベント時間** を選択。 
...
1. Azure portal で、**「az30304b-logicapp1」** ブレードに移動し、**「概要」** セクションで、**「トリガーの履歴」** を選択します。 
...
1. 「イベント」** ブレードで、**「+ イベント サブスクリプション」** を選択します。
...
1. **「イベント サブスクリプションの作成」** ブレードで、次の設定を指定し、**「作成」** を選択します。
    ...
    | イベントの種類のフィルター | **Resource Write Success**、**Resource Delete Success**、**Resource Action Success** |
    | エンドポイントの種類 | **webhook** | 
